...
class BillingTest {
	...
	void whenProductIsNotImportedNorFoodNorMedicineNorBookThenTaxIsTenPercent() {
		...
		assertEquals(0.1 * myProduct.getPrice(), new TaxCalculator().getTax(Stream.of(myProduct).collect(Collectors.toList())));
	}
	...
	void whenProductIsImportedAndNotFoodNorMedicineNorBookThenTaxIsFifteenPercent() {
		...
		assertEquals(0.15 * myProduct.getPrice(), new TaxCalculator().getTax(Stream.of(myProduct).collect(Collectors.toList())));
	}

	@Test
	void whenProductIsFoodNotImportedThenTaxIsZero() {
		Product myProduct = new Product("boite de chocolats", 10.0, false, "Food");
		assertEquals(0, new TaxCalculator().getTax(Stream.of(myProduct).collect(Collectors.toList())));
	}
	...
	void whenProductIsImportedFoodThenTaxIsFivePercent() {
		...
		assertEquals(0.05 * myProduct.getPrice(), new TaxCalculator().getTax(Stream.of(myProduct).collect(Collectors.toList())));
	}
	...
	void whenNotRoundedTaxIs4_1985ThenRoundedTaxIs4_2() {
		...
		assertEquals(4.2, new TaxCalculator().getTax(Stream.of(myProduct).collect(Collectors.toList())));
	}
	...
	void whenNotRoundedTaxIs5_555ThenRoundedTaxIs5_6() {
		...
		assertEquals(0.6, new TaxCalculator().getTax(Stream.of(myProduct).collect(Collectors.toList())));
	}
	
	@Test
	void whenNotRoundedTaxIs0_5ThenRoundedTaxIs0_5() {
		Product myProduct = new Product("Le Clean Code pour les nuls importé", 10.0, true, "Book");
		assertEquals(0.5, new TaxCalculator().getTax(Stream.of(myProduct).collect(Collectors.toList())));
	}
	...
	void whenProductsAreOneImportedPerfumeOnePerfumeOneHeachacheMedicineBoxOneImportedChocolateBoxThenTotalTaxIs6_70() {
		...
		assertEquals(6.70, new TaxCalculator().getTax(Stream.of(myProduct1,myProduct2,myProduct3,myProduct4).collect(Collectors.toList())));
	} 
